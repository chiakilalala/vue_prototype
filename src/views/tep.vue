<template>
  <v-container fluid >
    <v-row no-gutters  align="center">
      <v-col cols="12" md="12" class="border-left-primary">
        <v-card class="pa-1" outlined>
          <v-row no-gutters>
            <v-col cols="2">
              <v-subheader :inset="inset">業務類別:</v-subheader>
            ...
            <v-col cols="2" md="2">
               <v-subheader :inset="inset">業務項目:</v-subheader>
            </v-col>
            <v-col cols="3" md="3">
              <v-select
                :items="transactions"
    ...
    <v-row no-gutters>
      <v-col cols="12" md="12" v-show="!isShow" class="my-1 border-left-primary">
        <v-card class="pa-1 " outlined v-if="selectedTransaction !== null" >
         <v-subheader>
            ...
            <!-- 現金類 台幣現金取款（金融卡ATM交易）-->
            <span v-if="selectedCategory=='現金類' && selectedTransaction=='台幣現金取款（金融卡ATM交易）' " class="font-weight-regular subheading">
              <StepCash :step="step"> </StepCash>
            </span>

                <!-- 現金類 台幣現金取款（Bancs無摺交易）-->
            <span v-if="selectedCategory=='現金類' && selectedTransaction=='台幣現金取款（Bancs無摺交易）' " class="font-weight-regular subheading">
                  <StepBancs :step="step"> </StepBancs>
            </span>
            <!-- 非現金類 -->
      ...
      <v-col cols="12" md="12" v-show="!isShow" class="">
        <v-card class="pa-1" outlined v-if="selectedTransaction !== null" >
          <v-row no-gutters>
            ...
            <!-- 申請類 -->
            <v-col v-if="selectedCategory=='申請類' && selectedTransaction=='約定轉入帳號' " cols="6" md="6" class>
              <span v-if="this.step ==1 " style="font-size:16px">
                <span class="stepball primary mr-1">1</span>提示客戶證件掃描
              </span>
              <span v-if="this.step ==2 "  style="font-size:16px">
  ...
  <!-- 申請類 變更客戶基本資料-->
              <v-col v-if="selectedCategory=='申請類' && selectedTransaction=='變更客戶基本資料' " cols="6" md="6" class>
              <span v-if="this.step ==1 " style="font-size:16px">
                <span class="stepball primary mr-1">1</span>提示客戶證件掃描
              </span>
              <span v-if="this.step ==2 "  style="font-size:16px">
        ...
        <!-- 申請類 提款卡申請/終止非約定轉帳-->
            <v-col v-if="selectedCategory=='申請類' && selectedTransaction=='提款卡申請/終止非約定轉帳' " cols="6" md="6" class>
        ...
        <!-- 申請類 存摺取款密碼變更/刪除-->
      <v-col v-if="selectedCategory=='申請類' && selectedTransaction=='存摺取款密碼變更/刪除' " cols="6" md="6" class>
            ...
            <!-- 現金類 -->
                ...
                <!-- 現金類 台幣現金存入（金融卡ATM交易）-->
            <v-col v-if="selectedCategory=='現金類' && selectedTransaction=='台幣現金存入（金融卡ATM交易）' " cols="6" md="6" class>      
              <span v-if="this.step ==1 " style="font-size:16px">
            ...
            <!-- 現金類 台幣現金存入（Bancs無摺交易）-->
              <v-col v-if="selectedCategory=='現金類' && selectedTransaction=='台幣現金存入（Bancs無摺交易）' " cols="6" md="6" class>      
              <span v-if="this.step ==1 " style="font-size:16px">
              ...
              <span v-if="this.step ==4 "  style="font-size:16px">
                ...
                <span v-if="this.step ==5 "  style="font-size:16px">
                <span class="stepball primary mr-1">5</span> 交易執行(請客戶輸入密碼，有取款密碼時) 引導客戶存入現鈔               
              </span>
              <span v-if="this.step ==6 "  style="font-size:16px">
            ...
            <!-- 現金類 台幣現金取款（金融卡ATM交易）-->
            <v-col v-if="selectedCategory=='現金類' && selectedTransaction=='台幣現金取款（金融卡ATM交易）' " cols="6" md="6" class>      
              <span v-if="this.step ==1 " style="font-size:16px">
              ...
              <span v-if="this.step ==4 "  style="font-size:16px">
                <span class="stepball primary mr-1">4</span> 引導客戶取款提醒收據（交易超過十萬元，送出給主管覆核）
              </span>
            </v-col>
            <!-- 現金類 台幣現金取款（Bancs無摺交易）-->
              <v-col v-if="selectedCategory=='現金類' && selectedTransaction=='台幣現金取款（Bancs無摺交易）' " cols="6" md="6" class>      
              <span v-if="this.step ==1 " style="font-size:16px">
                <span class="stepball primary mr-1">1</span>確認客戶是否已填寫表單 (若無引導客戶預填)
              </span>
              <span v-if="this.step ==2 " style="font-size:16px">
              ...
              <span v-if="this.step ==6 "  style="font-size:16px">
                <span class="stepball primary mr-1">6</span> 交易覆核並提供收據(金額 100萬含以上需進行覆驗印)
              </span>
              <span v-if="this.step ==7"  style="font-size:16px">
                <span class="stepball primary mr-1">7</span> 掃描交易憑條副本（如客戶要求）
              </span>
              ...
              <!-- 非現金類 -->
            <v-col v-if="selectedCategory=='非現金類' && selectedTransaction=='台幣轉帳' " cols="6" md="6" class>      
              <span v-if="this.step ==1 " style="font-size:16px">
              ...
              <span v-if="this.step ==4 "  style="font-size:16px">
                <span class="stepball primary mr-1">4</span> 交易超過三萬元提供證件掃描（本行客戶得以詢問個資方式確認身份）
              ...
              <!-- 非現金類 -->
                <v-col v-if="selectedCategory=='非現金類' && selectedTransaction=='台幣匯款' " cols="6" md="6" class>      
              <span v-if="this.step ==1 " style="font-size:16px">
                <span class="stepball primary mr-1">1</span>確定客戶預填表單
              </span>
                <span v-if="this.step ==2 " style="font-size:16px">
...
<v-col cols="3" md="3">
    ...
    <v-row justify="end">
      <v-col cols="5" md="5">
        <v-btn @click.prevent="back(step)" style="margin-right: 4px;" :disabled="currentUnit === null || step<2">上一步</v-btn>
        <v-btn
          ...
          || (step>6 && selectedCategory=='申請類' && selectedTransaction=='存摺取款密碼變更/刪除') 
          ...
          || (step>4 && selectedCategory=='現金類' && selectedTransaction=='台幣現金取款（金融卡ATM交易）')
          || (step>6 && selectedCategory=='現金類' && selectedTransaction=='台幣現金取款（Bancs無摺交易）')  
        ...
        && (this.selectedCategory!='現金類' && this.selectedTransaction!='台幣現金取款（金融卡ATM交易）')
        && (this.selectedCategory!='現金類' && this.selectedTransaction!='台幣現金取款（Bancs無摺交易）')
        ...
        || (this.step ==5 && this.selectedCategory=='現金類' && this.selectedTransaction=='台幣現金取款（金融卡ATM交易）')
        || (this.step ==7 && this.selectedCategory=='現金類' && this.selectedTransaction=='台幣現金取款（Bancs無摺交易）')
        || (this.step ==7 && this.selectedCategory=='申請類' && this.selectedTransaction=='約定轉入帳號')
        || (this.step ==7 && this.selectedCategory=='申請類' && this.selectedTransaction=='存摺取款密碼變更/刪除')
        || (this.step ==7 && this.selectedCategory=='申請類' && this.selectedTransaction=='提款卡申請/終止非約定轉帳')
...
export default {
  ...
  computed: {
    ...
    steps() {
      ...
      else if(this.selectedCategory=='現金類' && this.selectedTransaction=='台幣現金取款（金融卡ATM交易）'){
        return 5;
      }
      else if(this.selectedCategory=='現金類' && this.selectedTransaction=='台幣現金取款（Bancs無摺交易）'){
        return 7;
      }
